...
export function JustAddedProducts({ initialProducts }: { initialProducts: any }) {
    /* const [isClient, setIsClient] = useState(false) */

    //console.log(featuredProducts,';;;;;;;;;;');
    return (
        <section className="w-full px-4 md:px-8 py-10 bg-buyer-soft/40 ">
            <div className="w-full container mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
                    <div>
                        <h2 className="text-2xl lg:text-3xl font-bold tracking-tight">Just Added</h2>
                        <p className="text-sm text-muted-foreground mt-1">New listings from the last 5 days.</p>
                    </div>
                ...
                <MyCarousel sliderToShow={6} breackpoints={[
                    ...
                    {initialProducts.map((slide, i) => (
                        <ItemFeaturedProduct key={i} {...slide} isNew={true} className="min-w-[90vw] min-[460px]:min-w-0" />
                    ))}
                </MyCarousel>
            </div>
        </section>
